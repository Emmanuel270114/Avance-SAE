...
def get_matricula_metadata_from_sp(
    ...
    default_periodo: Optional[str] = None,
    usuario: str = 'sistema',
    host: str = 'localhost'
) -> Dict[str, Any]:
    """
...
def execute_sp_valida_matricula(
    ...
    """
    # El SP compara niv.Nivel = @NNivel, por lo que debemos enviar el NOMBRE del nivel (p.ej., 'Superior').
    # Resolver a nombre aunque recibamos un Id.
    try:
        # Si es numérico, buscar el nombre
        nivel_id = int(nivel)
        r = db.execute(text("SELECT Nivel FROM [SAE].[dbo].[Cat_Nivel] WHERE Id_Nivel = :id"), {'id': nivel_id}).fetchone()
        nivel_literal = str(r[0]) if r else str(nivel)
    except Exception:
        # Ya es un nombre; usarlo tal cual
        nivel_literal = str(nivel)
...
def execute_sp_rechaza_matricula(
    ...
    """
    # Este SP también usa el nombre del nivel; resolver a nombre si viene Id
    try:
        nivel_id = int(nivel)
        r = db.execute(text("SELECT Nivel FROM [SAE].[dbo].[Cat_Nivel] WHERE Id_Nivel = :id"), {'id': nivel_id}).fetchone()
        nivel_literal = str(r[0]) if r else str(nivel)
